 ...
 */
export const CAPABILITIES = ['private-data', 'untrusted-content', 'external-egress'];
/**
 * 会话级滑动窗口能力追踪器。
...
export class TrifectaTracker {
    ...
    record(capability, tool, evidence) {
    ...
    prune() {
        ...
        let firstLive = 0;
        while (firstLive < this.events.length && this.events[firstLive].ts < cutoff)
            firstLive++;
        if (firstLive > 0)
            this.events.splice(0, firstLive);
    ...
    status() {
        this.prune();
        const counts = { 'private-data': 0, 'untrusted-content': 0, 'external-egress': 0 };
        const latest = {};
...
export function classifyCapabilities(obs) {
    const caps = new Set();
    for (const cat of obs.categories ?? []) {
        const cap = CATEGORY_CAPABILITY[cat];
        if (cap)
            caps.add(cap);
    }
    if (obs.secret)
        caps.add('private-data');
    if (obs.network)
        caps.add('external-egress');
    if (obs.tool) {
