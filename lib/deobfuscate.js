...
function decodeEntitiesOnce(s) {
    return s.replace(ENTITY_RE, (m) => {
        ...
        if (body[0] === '#') {
            ...
            // 拒绝控制字符（保留 \t\n\r）与越界码点
            if (!Number.isFinite(cp) || cp > 0x10ffff || (cp < 0x20 && cp !== 9 && cp !== 10 && cp !== 13))
                return m;
            try {
...
function decodeTagsBlock(s) {
    if (!TAGS_ANY_RE.test(s))
        return s;
    let out = '';
    for (const ch of s) {
        ...
        if (cp >= 0xE0020 && cp <= 0xE007E)
            ...
            continue; // LANGUAGE TAG / CANCEL TAG
        else
            out += ch;
    }
    return out;
...
export function findBase64Segments(text, maxSegments = 2) {
    const hits = [];
 ...
 */
export function expandTexts(input) {
    const { canonical, transforms } = normalizeText(input);
    const variants = [{ text: input, via: [] }];
    if (canonical !== input)
        variants.push({ text: canonical, via: transforms });
