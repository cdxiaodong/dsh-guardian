...
export const DEFAULT_RULES: Rule[] = [
  ...
  // ── RCE：反序列化 / 动态执行（提炼自 Microsoft《When prompts become shells》2026-05 披露：
  //    提示注入经 agent 框架的不安全反序列化与动态执行路径升级为宿主 RCE，
  //    含 Semantic Kernel CVE-2026-26030 与 agent checkpoint 反序列化案例）──
  { id: 'RCE-001', category: 'rce', weight: 0.95, pattern: /\b(c?pickle|dill)\.loads?\s*\(/i, reason: 'RCE 向量：Python pickle 反序列化（构造 payload 即任意代码执行）', action: 'deny' },
  { id: 'RCE-002', category: 'rce', weight: 0.95, pattern: /\bmarshal\.loads?\s*\(/i, reason: 'RCE 向量：marshal 反序列化', action: 'deny' },
  { id: 'RCE-003', category: 'rce', weight: 0.9, pattern: /\byaml\.load\s*\((?![^)]*Loader\s*=\s*(?:yaml\.)?(?:SafeLoader|BaseLoader|CSafeLoader))/i, reason: 'RCE 向量：yaml.load 未指定 SafeLoader（可构造任意对象）', action: 'deny' },
  { id: 'RCE-004', category: 'rce', weight: 0.9, pattern: /\b(eval|exec)\s*\(\s*['"`]?\w*(input|request|payload|content|response|message|body|cmd|command|code)\b/i, reason: 'RCE 向量：eval/exec 执行模型可控输入', action: 'deny' },
  { id: 'RCE-005', category: 'rce', weight: 0.8, pattern: /child_process.{0,30}\b(exec|execSync)\b\s*\(|\bexecSync\s*\(\s*`|spawn.{0,30}shell\s*:\s*true/i, reason: 'RCE 向量：child_process shell 模式执行（模板字符串/spawn shell:true）', action: 'block' },
  { id: 'RCE-006', category: 'rce', weight: 0.9, pattern: /\b(torch|joblib)\.load\s*\(/i, reason: 'RCE 向量：模型/checkpoint 文件反序列化（pickle 后端，恶意模型即代码执行）', action: 'block' },
  { id: 'RCE-007', category: 'rce', weight: 0.8, pattern: /\bnew\s+Function\s*\(|\bFunction\s*\(\s*["'`]return/i, reason: 'RCE 向量：new Function 动态构造代码', action: 'deny' },
  { id: 'RCE-008', category: 'rce', weight: 0.7, pattern: /\bdeserialize|unserialize|readObject|ObjectInputStream\b/i, reason: 'RCE 向量：Java/PHP 反序列化入口', action: 'block' },
  { id: 'RCE-009', category: 'rce', weight: 0.8, pattern: /torch\.load.{0,40}weights_only\s*=\s*false|pickle\.Unpickler\s*\(/i, reason: 'RCE 向量：显式放宽反序列化安全约束', action: 'block' },
]
