...
export function scanNetworkTarget(text: string): Array<{ ruleId: string; reason: string }> {
  const hits: Array<{ ruleId: string; reason: string }> = []
  for (const rule of SSRF_RULES) {
    rule.pattern.lastIndex = 0
    if (rule.pattern.test(text)) hits.push({ ruleId: rule.id, reason: rule.reason })
  }
  return hits
}
